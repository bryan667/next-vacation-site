...
export function ParallaxStorySection({ id, label, title, text, image, align = "left" }: ParallaxStorySectionProps) {
  return (
    <section className="relative flex min-h-screen items-center overflow-hidden" id={id}>
      <div
      ...
      >
        <span className="mb-4 block text-xs uppercase tracking-[4px] text-sand-gold">{label}</span>
        <h2 className="mb-6 text-[clamp(2.5rem,5vw,4rem)] leading-[1.2] [font-family:var(--font-caveat)]">{title}</h2>
